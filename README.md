...
- `grandpa` & `aura` consensus were removed. Instead, [`instant-seal`/`manual-seal`](https://github.com/AstarNetwork/test-node#consensus-manual-seal--instant-seal) & [`delayed-finalize`](https://github.com/AstarNetwork/test-node#consensus-delayed-finalize) are used.
...
## Installation
### Download Binary
The easiest method of installation is by downloading and executing a precompiled binary from the [Release Page](https://github.com/AstarNetwork/test-node/releases)

### Build Locally
If you would like to build the source locally, you should first complete the [basic Rust setup instructions](https://github.com/AstarNetwork/test-node/blob/main/docs/rust-setup.md).
...
```bash
./target/release/test-node purge-chain
```

> The **alice** development account will be the authority and sudo account as declared in the
> [genesis state](https://github.com/AstarNetwork/test-node/blob/main/node/src/chain_spec.rs#L44).
...
(`cargo build --release && ./target/release/test-node --dev --ws-external`)
by appending your own. A few useful commands are shown below:

```bash
# Run Substrate node without re-compiling
./scripts/docker_run.sh ./target/release/test-node --ws-external

# Purge the local dev chain
./scripts/docker_run.sh ./target/release/test-node purge-chain
...
```bash
./target/release/test-node --finalize-delay-sec 5
```
